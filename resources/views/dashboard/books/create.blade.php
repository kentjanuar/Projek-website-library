...
{{-- <form class="max-w-sm mx-auto" action="/dashboard/books" method="post" enctype="multipart/form-data">
    @csrf
    ...
    <div class="mb-5">
        ...
        placeholder="title" required value="{{ old('title') }}" autofocus/>
    ...
    <div class="mb-5">
        ...
        <input type="text" id="published_year" name="published_year" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{ old('published_year') }}" placeholder="Your address" required />
...
<form class="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg my-14" action="/dashboard/books" method="post" enctype="multipart/form-data">
    @csrf

    <h2 class="text-2xl font-bold text-gray-800 dark:text-white mb-6 text-center">Add a New Book</h2>

    <!-- Book Title -->
    <div class="mb-5">
        <label for="title" class="block mb-2 text-sm font-medium text-gray-900">Book title</label>
        <input type="title" name="title" id="title" title="title" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Title" required value="{{ old('title') }}" autofocus/>
        @error('title')
            <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="mb-5">
        ...
        <input type="text" name="author" id="author" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Author" value="{{ old('author') }}" required/>
        @error('author')
            <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="flex mb-5 space-x-4">
        <div class="flex-1 mr-4">
            ...
            <input type="text" id="publisher" name="publisher" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ old('publisher') }}" placeholder="Publisher" required />
            @error('publisher')
                <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="w-1/3">
            ...
            <input type="text" id="published_year" name="published_year" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ old('published_year') }}" placeholder="Published Year" required />
            @error('published_year')
                <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex-1">
            ...
            <div class="flex space-x-4 mt-4">
                <div class="flex items-center">
                    <input type="radio" id="condition_good" name="condition" value="Good" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300" {{ old('condition') == 'Good' ? 'checked' : '' }}>
                    <label for="condition_good" class="ml-2 text-sm font-medium text-gray-900">Good</label>
                </div>
                <div class="flex items-center">
                    <input type="radio" id="condition_broken" name="condition" value="Broken" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300" {{ old('condition') == 'Broken' ? 'checked' : '' }}>
                    <label for="condition_broken" class="ml-2 text-sm font-medium text-gray-900">Broken</label>
                </div>
                <div class="flex items-center">
                    <input type="radio" id="condition_in_repair" name="condition" value="In Repair" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300" {{ old('condition') == 'In Repair' ? 'checked' : '' }}>
    ...
    <div class="mb-5">
        ...
        <textarea id="description" name="description" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full h-40 p-2.5" placeholder="Your description" required>{{ old('description') }}</textarea>
        @error('description')
            <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="flex justify-end">
        <button type="submit" class="text-white bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:outline-none focus:ring-indigo-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">Add Book</button>
    </div>
